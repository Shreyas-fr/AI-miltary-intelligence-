...
if "selected_city" not in st.session_state:
    st.session_state.selected_city = "All (Country-level)"
...
cities_df = get_cities(selected_country)
city_options = ["All (Country-level)"] + cities_df["city"].tolist()

if st.session_state.selected_city not in city_options:
    st.session_state.selected_city = "All (Country-level)"

def on_city_change():
    chosen_city = st.session_state.selected_city
    if chosen_city != "All (Country-level)":
        row_c = cities_df[cities_df["city"] == chosen_city]
        if not row_c.empty:
            st.session_state.mission_lat = float(row_c.iloc[0]["med_lat"])
            st.session_state.mission_lon = float(row_c.iloc[0]["med_lon"])
    else:
        # Revert to country centroid
        chosen = st.session_state.selected_country
        row_c = country_df[country_df["country_txt"] == chosen]
        if not row_c.empty:
            st.session_state.mission_lat = float(row_c.iloc[0]["med_lat"])
            st.session_state.mission_lon = float(row_c.iloc[0]["med_lon"])
...
selected_city = st.sidebar.selectbox(
    ...
    help="Optional: Set the map view to a specific city in this country."
)
...
with tab_mission:
    ...
    mission_center_df["tooltip_title"] = "Target Coordinate"
    with st.spinner("Reverse geocoding target coordinate..."):
        resolved_center = reverse_geocode(lat, lon)
    mission_center_df["tooltip_loc"] = f"Location: {resolved_center}"
    mission_center_df["tooltip_det"] = f"Search Radius: {radius_km} km"
    ...
    st.markdown("*Map data & reverse geocoding © [OpenStreetMap](https://www.openstreetmap.org/copyright) contributors*")
