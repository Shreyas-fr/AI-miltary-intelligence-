...
from utils.weather_utils import fetch_weather_by_coords, assess_operational_impact
...
with st.spinner("Fetching current weather from Open-Meteo..."):
...
if weather:
    ...
    with st.spinner("Resolving location name..."):
        resolved_location = reverse_geocode(lat, lon)

    with col_desc:
        st.markdown(f"### 📍 Location: {resolved_location}")
        st.markdown(
    ...
    # Attribution
    st.markdown("---")
    st.markdown("*Weather data © [Open-Meteo.com](https://open-meteo.com/)*")

# 8. When API key is NOT available or fetch returns None
else:
    st.warning("⚠️ Open-Meteo API is currently unreachable. Weather intelligence is temporarily degraded.")
