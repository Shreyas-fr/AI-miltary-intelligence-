...
**Claim:** Random Forest Classifier Accuracy is **79.82%**.
...
### 1a. SHAP Explainability Scenarios
**Claim:** The model bases its predictions on logical tactical indicators.
*   **Source:** Post-training SHAP TreeExplainer run against 5 diverse operational scenarios.
*   **Results (Top 3 features driving the prediction):**
    1. **Afghanistan (Taliban):** Predicted *Bombing/Explosion* (Driven by: `weaptype`, `success`, `nwound`)
    2. **Iraq (ISIL):** Predicted *Bombing/Explosion* (Driven by: `weaptype`, `nwound`, `success`)
    3. **India (Unknown):** Predicted *Armed Assault* (Driven by: `nkill`, `weaptype`, `targtype`)
    4. **Pakistan (TTP):** Predicted *Bombing/Explosion* (Driven by: `weaptype`, `success`, `nwound`)
    5. **Somalia (Al-Shabaab):** Predicted *Bombing/Explosion* (Driven by: `weaptype`, `success`, `nwound`)
*   **The Defense:** "Our SHAP analysis proves the model is not a black box. In India, where firearms and targeted killings are common, the model heavily weights the casualty count (`nkill`) and weapon type. In the Middle East and Africa, it correctly identifies that explosive tactics dominate, weighting `weaptype` heavily towards bombings."
...
## 6. SIGINT / RF Spectrum Simulation
**Claim:** The spectrum analyzer displays signal density and downconversion math.
*   **The Defense:** "We are fully transparent: this is a **Simulation** mode showing synthetic signal generation since no physical SDR hardware (e.g. HackRF) is connected to the presentation machine. We place a persistent warning banner at the top of the view. We decided against installing a full local GNU Radio build environment for safety and time reasons—as GNU Radio has over 60 dependencies on macOS (including outdated Qt5 libraries) that take hours to compile and often fail on Apple Silicon. Instead, we implemented a Python-only ZMQ publisher mimicking the downconverter offset math ($15\text{ GHz} \rightarrow 150\text{ MHz}$). This proves the modular ZMQ data extraction pipeline works cleanly in Streamlit without blocking the UI thread."
